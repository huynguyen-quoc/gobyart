window.conApp = {};
window.conApp.inProgress = false;
...
conApp.initValidatorLanguage = function() {
    //noinspection JSUnresolvedVariable,JSUnresolvedFunction
    Parsley.addMessages('vi', {
        defaultMessage: "Giá trị không chính xác.",
        type: {
        ...
        notblank:       "Xin hãy nhập thông tin",
        required:       "Xin hãy nhập thông tin",
        pattern:        "Thông tin sai định dạng",
...
conApp.ajaxCall = function(request) {
   if (window.conApp.inProgress) return;
   window.conApp.inProgress = true;
   var headers = {
       'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
   };
   if($.cookie('api_token')){
       headers.Authorization =  'Bearer ' +  $.cookie('api_token');
   }
   if(request.headers) {
       request.headers = $.extent(headers, request.header);
   }else{
   ...
   request.error = function (errorData) {
     window.conApp.inProgress = false;
     console.log(errorData);
     if(errorData.status === 401){
       window.location.href = errorData.responseJSON.intended;
     }else {
       error(errorData)
     }
   };
...
conApp.showSpin = function() {
    $('.load-spinner').toggleClass('hide show', 'medium', "easeOutSine");
};

conApp.hideSpin = function() {
    $('.load-spinner').toggleClass('show hide', 'medium', "easeOutSine");
};
